    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        <UCard v-for="animal in localAnimals" :key="animal.id" @click="toggleSelect(animal.id)" :class="[
            ...
            <div class="relative h-48 w-full overflow-hidden rounded-t-3xl">
                ...
                <div v-else class="flex items-center justify-center h-full bg-purple-200/40 text-purple-500 text-sm">
                    Pas d’image
                </div>
            </div>
...
const { lodgerCount } = useReservationFormStore()
...
const toggleSelect = (id: string) => {
    ...
    } else if (selectedIds.value.length + 1 <= lodgerCount) {
        selectedIds.value.push(id)
    } else {
        // TODO handle toast error here.
        alert("You have reached the maximum number of selected animals for this reservation.")
    }
